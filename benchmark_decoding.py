...
if USE_TRITON:
    ...
    def paged_attention_decode_triton(*args, **kwargs):
...
def naive_decode_attention(
    ...
    for i in range(batch_size):
        ...
        if seq_k_list:
            all_k.append(torch.cat(seq_k_list, dim=0)[:seq_len])
            all_v.append(torch.cat(seq_v_list, dim=0)[:seq_len])
...
def benchmark(batch_size, seq_len, num_heads=32, num_kv_heads=8, head_dim=128, block_size=16, num_iterations=100):
    print(f"\n{'=' * 70}")
    print(f"Device={DEVICE}, batch_size={batch_size}, seq_len={seq_len}, num_heads={num_heads}")
    ...
    for _ in range(10):
        _ = naive_decode_attention(q, k_cache, v_cache, block_tables, context_lens, scale, num_heads, num_kv_heads, head_dim, block_size)
    sync()
    start = time.perf_counter()
    for _ in range(num_iterations):
        _ = naive_decode_attention(q, k_cache, v_cache, block_tables, context_lens, scale, num_heads, num_kv_heads, head_dim, block_size)
    sync()
    results["Naive PyTorch"] = (time.perf_counter() - start) / num_iterations
    print(f"   Time: {results['Naive PyTorch'] * 1000:.3f}ms")

    print("\n2. Testing Optimized PyTorch implementation...")
    for _ in range(10):
        _ = decode_torch_optimized(q, k_cache, v_cache, block_tables, context_lens, scale, num_heads, num_kv_heads, head_dim, block_size)
    sync()
    start = time.perf_counter()
    for _ in range(num_iterations):
        _ = decode_torch_optimized(q, k_cache, v_cache, block_tables, context_lens, scale, num_heads, num_kv_heads, head_dim, block_size)
    sync()
    results["Optimized PyTorch"] = (time.perf_counter() - start) / num_iterations
    print(f"   Time: {results['Optimized PyTorch'] * 1000:.3f}ms")

    if USE_TRITON:
        print("\n3. Testing Triton implementation...")
        for _ in range(10):
            _ = paged_attention_decode_triton(q, k_cache, v_cache, block_tables, context_lens, scale, num_heads, num_kv_heads, head_dim, block_size)
        sync()
        start = time.perf_counter()
        for _ in range(num_iterations):
            _ = paged_attention_decode_triton(q, k_cache, v_cache, block_tables, context_lens, scale, num_heads, num_kv_heads, head_dim, block_size)
        sync()
        results["Triton"] = (time.perf_counter() - start) / num_iterations
        print(f"   Time: {results['Triton'] * 1000:.3f}ms")
    else:
        print("\n3. Triton implementation skipped: requires triton + supported device: cuda/xpu.")
...
if __name__ == "__main__":
    ...
    print("Comparing: Naive PyTorch | Optimized PyTorch | optional Triton")
    print("=" * 70)
